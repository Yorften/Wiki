            ...
            <div id="tags" class="flex flex-col justify-center items-center w-full h-full p-1 md:p-4">
                <div class="w-full px-2">
                    <div id='recipients' class="p-2 pb-8 rounded shadow-xl bg-white">
                        <?php if (!isset($tags)) { ?>
                            <p class="w-full h-full flex items-center justify-center">No tags found</p>
                        <?php } else { ?>
                            <table id="table" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                                ...
                                <tbody>
                                    ...
                                    foreach ($tags as $tag) { ?>
                                        <tr>
                                            <th><?= $tag->getId() ?></th>
                                            <th id="tagName<?= $tag->getId() ?>"><?= $tag->getName() ?></th>
                                            <th>
                                                <div class="mx-auto flex items-center justify-evenly w-full gap-10">
                                                    <button onclick="showTagDetails(<?= $tag->getId() ?>)" class="inline-flex items-center px-4 py-2 bg-white hover:bg-gray-200 shadow-lg text-sm font-medium rounded-md">
